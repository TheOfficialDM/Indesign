...
if (!app.documents.length || parseInt (app.version) < 5) {
	exit();
}
...
function main (kernInterface) {
	var resetDialog = setFCDialog();
	...
	if (scope instanceof Story || scope instanceof Document || scope.hasOwnProperty('baseline')) {
...
function kernDocument (doc, kernfolder, logfile) {
	var str = doc.hasOwnProperty('name') ? doc.name : '';
	var mess = create_message ('', 40);
	mess.show();
...
function kern (doc, section, mess, font) {
	...
	// See if we can split k.style on a tab. If we can, i.e. if styles.length > 1, 
	// then we have two style names, which requires different action
...
function same_style (doc, srch, kv, mess) {
	...
	for (var i = 0; i < pairs.length; i++) {
		try {
			if (srch[0] == '^') {  // The first character in a paragraph
				pairs[i].insertionPoints[0].contents = '\u00Ad';
				pairs[i].insertionPoints[1].kerningValue = Number (kv);
			} else if (kv.toLowerCase() == 'm') {
...
function diff_any (doc, srch, kv, style1, style2, mess) {
	kv = kv.toLowerCase();
	mess.txt.text = srch;
	app.findGrepPreferences.findWhat = srch[0];
	var ch = doc.findGrep();
	for (var i = 0; i < ch.length; i++) {
		if (ch[i].parent.texts[0].characters[ch[i].index+1].contents === srch[1]) {
			ch[i].insertionPoints[1].kerningValue = Number (kv);
		}
	}
}
...
function kern_different_styles (doc, style_array, pairs, mess, font) {
	...
	for (var i = 0; i < pairs.length; i++) {
		...
		temp = pairs[i].split ('\t');
		if (style_2.indexOf('ANY') > -1) {
			diff_any (doc, temp[0], temp[1], style_1, style_2, mess);
		} else {
...
function document_fonts (doc) {
	...
	} else if (doc.hasOwnProperty('baseline')) {
		var f = doc.parentStory.parent.fonts.everyItem().fontFamily;
	} else {
...
function get_folder () {
	...
	if (f.exists) {
		...
		if (!Folder(dir).exists) {
			errorExit ('The folder ' + dir + ' listed in\rkern_folder.txt does not exist.');
		}
		// it does, return it
		return dir;
	} else {
...
function userInput () {
	var w = new Window ('dialog', 'Kern', undefined, {closeButton: false});
		...
		var p = w.add ('panel {alignChildren: "left"}');
			...
			p.add ('radiobutton {text: "Selected text"}');
